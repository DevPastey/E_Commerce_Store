        ...
        <nav className="flex gap-4 py-2 items-center justify-center text-gray-400">
          ...
          {user && (
            <Link to={"/cart"} className="relative flex items-center gap-1 group hover:text-emerald-400 transition duration-300 ease-in-out">
              ...
              { cart.length > 0 && (<span className="absolute -top-1 -left-2 bg-primary-green/50 text-white rounded-full px-1.5 py-px text-xs group-hover:bg-emerald-400 transition duration-300 ease-in-out">{cart.length}</span>)
                } 
           </Link>
          )}
